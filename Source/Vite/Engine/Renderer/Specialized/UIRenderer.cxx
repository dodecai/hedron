...
void ClipRect::Activate() {
    ...
    if (current.Enabled) {
        auto &properties = mViewport->Properties();
        ...
        glScissor(static_cast<GLint>(x), static_cast<GLint>(properties.Size.Height - (y + height)), static_cast<GLsizei>(width), static_cast<GLsizei>(height));
    } else {
        glDisable(GL_SCISSOR_TEST);
    }
}

void ClipRect::Push(const Position2D &position, const Size2D &size) {
...
void ClipRect::PushCombined(const Position2D &position, const Size2D &size, Viewport *viewport) {
    mViewport = viewport;
    if (mCurrentIndex < 0) {
        Push(position, size);
        return;
...
void ClipRect::PushTransform(const Position2D &position, const Size2D &size) {
    if (!Validate()) return;
