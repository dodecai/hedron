...
/// 
template<typename T>
concept typename_uuid = std::is_same_v<std::string, T> || (std::is_unsigned_v<T> && sizeof(T) <= 16);
...
private:
    ...
    void Generate() {
        if constexpr (std::is_unsigned_v<T>) {
            ...
            std::uniform_int_distribution<std::uint64_t> distribution { 0, std::numeric_limits<uint64_t>::max() };
            std::array<std::uint8_t, 16> bytes;
            for (size_t i = 0; i < 16; i += 8) {
                std::uint64_t value = distribution(sGenerator);
